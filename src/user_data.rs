...
const REGISTER_QUERY: &'static str = "INSERT INTO users (id, localpart, passhash) VALUES ($1, $2, $3)";
                                 ...
                                 bcrypt::hash(&password, bcrypt::DEFAULT_COST)
                                      ...
                                      .then(move |hash_res| -> BoxFut {
                                          match hash_res {
                                              Ok(hash) => {
                                                  println!("{:?}", hash);
                                                  Box::new(run_on_main(&remote, |handle| {
                                                      tokio_postgres::Connection::connect(db_params, tokio_postgres::TlsMode::None, &handle)
                                              ...
                                              Err(err) => {
                                                  eprintln!("{:?}", err);
                                                  Box::new(future::ok(ErrorBody::INTERNAL_ERROR.to_response()))
                                              }
                                          }
                                      }))
                         },
